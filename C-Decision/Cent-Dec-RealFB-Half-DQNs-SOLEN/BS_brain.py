...
class BS:
    ...
    def _create_model(self):
        ...
        Num_Inner_Layer_1 = 16
        Num_Inner_Layer_2 = 32
        Num_Inner_Layer_3 = 16
        ...
        BS_Dense1 = Dense(600, activation='relu', name='BS_Dense1')(BS_Input)
        BS_Dense2 = Dense(400, activation='relu', name='BS_Dense2')(BS_Dense1)
        BS_DNN = Dense(300, activation='relu', name='BS')(BS_Dense2)
...
class Agent:
    ...
    def run(self, num_episodes, num_test_step, opt_flag):
        ...
        # add spectrum sharing adapted from SOLEN scheme which is simply termed as SOLEN
        SOLEN_Flag = True

        if SOLEN_Flag:
            SOLEN_Expect_Return = np.zeros(self.num_Episodes)
            SOLEN_Reward = np.zeros((self.num_Episodes, self.num_Test_Step))
            SOLEN_Per_V2V_Rate = np.zeros((self.num_Episodes, self.num_Test_Step, self.num_D2D))
            SOLEN_Per_V2I_Rate = np.zeros((self.num_Episodes, self.num_Test_Step, self.num_CH))
            SOLEN_Per_V2B_Interference = np.zeros((self.num_Episodes, self.num_Test_Step, self.num_CH))
            SOLEN_D2D_Action_Index = np.zeros((self.num_Episodes, self.num_Test_Step))
        ...
        for Episode_loop in range(self.num_Episodes):
            ...
            for Run_loop in range(self.num_Test_Step):
                ...
                # spectrum sharing scheme adapted from SOLEN
                if SOLEN_Flag:
                    # get all possible allocation schemes based on the allocated principle in SOLEN
                    Possible_Allocations = list(itertools.permutations(range(self.num_CH)))
                    Num_Possible_Allocations = len(Possible_Allocations)

                    SOLEN_Feasible_Reward = np.zeros(Num_Possible_Allocations)
                    Possible_V2V_Rate = np.zeros((Num_Possible_Allocations, self.num_D2D))
                    Possible_V2I_Rate = np.zeros((Num_Possible_Allocations, self.num_CH))
                    Possible_Interference = np.zeros((Num_Possible_Allocations, self.num_CH))

                    for S_loop in range(Num_Possible_Allocations):
                        Curr_Allocation = Possible_Allocations[S_loop]
                        Curr_Action = np.reshape(Curr_Allocation, (self.num_D2D, 1))
                        # Just compute the reward Not update the states
                        [V2V_Rate, V2I_Rate, Interference] = self.dump_act(Curr_Action)
                        # adopt weighted sum rate as the reward
                        Sum_V2I_Rate = np.sum(V2I_Rate)
                        ...
                        SOLEN_Feasible_Reward[S_loop] = Curr_Reward
                        Possible_V2V_Rate[S_loop, :] = np.sum(V2V_Rate, axis=1)
                        Possible_V2I_Rate[S_loop, :] = V2I_Rate
                        Possible_Interference[S_loop, :] = Interference

                    Curr_SOLEN_Reward = np.max(SOLEN_Feasible_Reward)

                    if Curr_SOLEN_Reward > 0:
                        Curr_SOLEN_Act_Index = np.argmax(SOLEN_Feasible_Reward)
                        SOLEN_Reward[Episode_loop, Run_loop] = Curr_SOLEN_Reward
                        SOLEN_Expect_Return[Episode_loop] += SOLEN_Reward[Episode_loop, Run_loop]
                        SOLEN_D2D_Action_Index[Episode_loop, Run_loop] = Curr_SOLEN_Act_Index
                        Curr_SOLEN_V2V_Rate = Possible_V2V_Rate[Curr_SOLEN_Act_Index, :]
                        Curr_SOLEN_V2I_Rate = Possible_V2I_Rate[Curr_SOLEN_Act_Index, :]
                        Curr_SOLEN_Interference = Possible_Interference[Curr_SOLEN_Act_Index, :]
                        SOLEN_Per_V2V_Rate[Episode_loop, Run_loop, :] = Curr_SOLEN_V2V_Rate
                        SOLEN_Per_V2I_Rate[Episode_loop, Run_loop, :] = Curr_SOLEN_V2I_Rate
                        SOLEN_Per_V2B_Interference[Episode_loop, Run_loop, :] = Curr_SOLEN_Interference

                if Opt_Flag:
                    # implement Optimal scheme via Brute Force Search

                    # initialize variables
                    Num_Possible_Action = self.num_CH ** self.num_D2D
                    Curr_Feasible_Reward = np.zeros(Num_Possible_Action)
                    BF_V2V_Rate = np.zeros((Num_Possible_Action, self.num_D2D))
                    BF_V2I_Rate = np.zeros((Num_Possible_Action, self.num_CH))
                    BF_Interference = np.zeros((Num_Possible_Action, self.num_CH))
        ...
        if RA_Flag and Opt_Flag:
            ...
            if RA_Flag and SOLEN_Flag:
                ...
                return Expect_Return, Reward, \
                       ...
                       RA_Expect_Return, RA_Reward, RA_Per_V2V_Rate, RA_Per_V2I_Rate, \
                       RA_Per_V2B_Interference, \
                       SOLEN_Expect_Return, SOLEN_Reward, SOLEN_Per_V2V_Rate, SOLEN_Per_V2I_Rate, \
                       SOLEN_Per_V2B_Interference
            else:
                if RA_Flag:
                    print('Finish Testing the C-Decision scheme with Real Feedback '
                          ' without Implementing Optimal Scheme!')
