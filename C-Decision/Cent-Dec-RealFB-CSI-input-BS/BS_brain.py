...
# take the channel gain from V2V link to V2I link （V2V transmitter  to the BS） as the input of the BS DQN directly
# thus, BS does not need to feed this channel gain back to all V2V links ---Nov. 17 2019
...
class BS:
    ...
    def __init__(self, num_d2d, input_d2d_info, num_d2d_feedback, num_d2d_neighbor, num_ch):
        ...
        # V2V DNN input: channel gain of V2V, interference on all channels and transmit power
        self.num_One_D2D_Input = ((input_d2d_info - 2) * self.num_CH + 1) * self.num_Neighbor
        # cross channel gain from V2V to BS
        self.num_V2V_2_BS_CH_Input = self.num_CH * self.num_Neighbor
    ...
    def _create_model(self):
        ...
        # add h_k, B[n] (channel gain from V2V transmitter to the BS) directly to the BS DQN

        Num_hkB_Input = self.num_V2V_2_BS_CH_Input * self.num_D2D
        V2V_2_BS_CH_Input = Input(shape=(Num_hkB_Input,), name='V2V_2_BS_CH_Input')

        BS_Input = keras.layers.concatenate([D1_Output, D2_Output, D3_Output, D4_Output, V2V_2_BS_CH_Input],
                                            name='BS_Input')

        BS_Dense1 = Dense(1200, activation='relu', name='BS_Dense1')(BS_Input)
        BS_Dense2 = Dense(800, activation='relu', name='BS_Dense2')(BS_Dense1)
        BS_DNN = Dense(600, activation='relu', name='BS')(BS_Dense2)

        Num_BS_Output = self.num_Output
        BS_output = Dense(Num_BS_Output, activation='linear', name='BS_output')(BS_DNN)

        # Define the model
        model = Model(inputs=[D1_Input, D2_Input, D3_Input, D4_Input, V2V_2_BS_CH_Input],
                      outputs=[BS_output])
...
class Agent:
    ...
    def generate_d2d_transition(self, num_transitions):
        ...
        # add h_k, B[n] (channel gain from V2V transmitter to the BS) directly to the BS DQN
        Num_hkB_Input = self.brain.num_V2V_2_BS_CH_Input * self.num_D2D
        # V2V_2_BS_CH_Input = Input(shape=(Num_hkB_Input,), name='V2V_2_BS_CH_Input')
        ...
        for self.train_step in range(num_transitions):
            ...
            for D2D_loop in range(self.num_D2D):
                ...
                D2D_State[D2D_loop, 2*CH_gain_Index: Num_One_D2D_Input] = Train_D2D_Power_State[D2D_loop, :]

            # here the really states for the whole neural network consists of States and BS_States
            # where States refers to all the states of V2Vs
            # whereas BS_States denotes all V2Vs' channel gain to the BS on all the channels
            States = np.reshape(D2D_State, [1, Num_States])
            BS_States = np.reshape(Train_D2D_V2I_CH_State, [1, Num_hkB_Input])
            ...
            for D2D_loop in range(self.num_D2D):
                ...
                D2D_Next_State[D2D_loop, 2 * CH_gain_Index: Num_One_D2D_Input] = Train_D2D_Power_State[D2D_loop, :]

            # the NEXT State
            States_ = np.reshape(D2D_Next_State, [1, Num_States])
            BS_States_ = np.reshape(Next_Train_D2D_V2I_CH_State, [1, Num_hkB_Input])

            # sample in (s, a, r, s_) format
            sample = [States, BS_States, Actions, Reward, States_, BS_States_]
    ...
    def replay(self):
        ...
        # one batch sample  {states, BS_states, action, reward, next_states, next_BS_states}
        batchLen = len(batch)
        ...
        # add h_k, B[n] (channel gain from V2V transmitter to the BS) directly to the BS DQN
        Num_hkB_Input = self.brain.num_V2V_2_BS_CH_Input * self.num_D2D
        # get the states and next states of BS that is cross channel gain from V2V to BS
        test_BS_states = np.zeros((batchLen, Num_hkB_Input))
        test_BS_states_ = np.zeros((batchLen, Num_hkB_Input))

        for Sample_loop in range(batchLen):
            test_states[Sample_loop, :, :] = np.reshape(batch[Sample_loop][0], [Num_D2D, Num_One_D2D_Input])
            test_BS_states[Sample_loop, :] = batch[Sample_loop][1]
            if batch[Sample_loop][3] is None:
                test_states_[Sample_loop, :, :] = np.zeros((Num_D2D, Num_One_D2D_Input))
                test_BS_states_[Sample_loop, :] = np.zeros((1, Num_hkB_Input))
            else:
                test_states_[Sample_loop, :, :] = np.reshape(batch[Sample_loop][4], [Num_D2D, Num_One_D2D_Input])
                test_BS_states_[Sample_loop, :] = batch[Sample_loop][5]
        ...
        BS_s = np.zeros([Num_Batch, Num_hkB_Input])
        BS_s_ = np.zeros([Num_Batch, Num_hkB_Input])

        for batch_Loop in range(batchLen):
            # fetch current sample(observation) from Replay Buffer
            # observation = {S, A, R, S_}
            # here S = {states, BS_states}, similarly we have S = {states, BS_states_},
            o = batch[batch_Loop]
            # s = o[0]
            # a = o[1]
            # r = o[2]
