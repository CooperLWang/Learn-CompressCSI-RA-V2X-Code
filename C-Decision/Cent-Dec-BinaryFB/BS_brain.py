# implement the DNN in Each D2D and the DQN at the BS for the C-Decision scheme with the Binary feedback
...
def grad_control_bin(x):
    # stop gradient of sign_binarize_input(x) in the backward pass
...
class BS:
    ...
    def _create_model(self):
        ...
        # implement fixed real feedback while varying the number of binary outputs for each D2D
        ...
        # to quantize the real output of D1_Output
        ...
        # to quantize the real output of D2_Output
        ...
        # to quantize the real output of D3_Output
        ...
        # to quantize the real output of D4_Output
...
class Agent:
    ...
    def generate_d2d_transition(self, num_transitions):
        ...
        for self.train_step in range(num_transitions):
            ...
            Train_V2V_Rate = np.sum(Train_D2D_Reward)
            # compute sum rate of V2I link
            Sum_V2I_Rate = np.sum(V2I_Rate)

            # normalize the V2V reward if necessary
            Norm_V2V_Reward = Train_V2V_Rate/V2V_Rate_max
            # compute the current reward
            Reward = v2v_weight * Norm_V2V_Reward + v2i_weight * Sum_V2I_Rate
            ...
            # add the sample (or transition) to the replay memory
    ...
    def replay(self):
        ...
        return Train_Result, Q_mean, Q_max_mean
    ...
    def train(self, num_episodes, num_train_steps):
        ...
        for Episode_loop in range(self.num_Episodes):
            ...
            for Iteration_loop in range(self.num_Train_Step):
                ...
                self.generate_d2d_transition(self.num_transition)

                # train the DNN-RL
                [Train_Result, Q_mean, Q_max_mean] = self.replay()

                # record the train loss
                Train_Loss[Episode_loop, Iteration_loop] = Train_Result.history['loss'][0]
                # record Q mean and Q_max mean respectively
            ...
            if (Episode_loop + 1) % Save_Model_Interval == 0:
                ...
                pickle.dump((Curr_Train_Episode,
                             ...
                             Curr_Train_Q_mean_per_Episode, Train_Q_mean,
                             Curr_Train_Q_max_mean_per_Episode, Train_Q_max_mean), file_to_open)
                file_to_open.close()
    ...
    def generate_d2d_initial_states(self):
        # generate initial states for C-Decision to run
        Train_D2D_CH_State = np.zeros((self.num_D2D, self.num_Neighbor, self.num_CH))
        Train_D2D_Interf_State = np.zeros((self.num_D2D, self.num_Neighbor, self.num_CH))
        Train_D2D_V2I_CH_State = np.zeros((self.num_D2D, self.num_CH))
        Fixed_Power = self.env.V2V_power_dB_List[self.env.fixed_v2v_power_index]
    ...
    def run(self, num_episodes, num_test_step, opt_Flag):
        ...
        # implement Optimal Scheme (Opt) via the Brute Force Search
        Opt_Flag = opt_Flag
        if Opt_Flag:
        ...
        if RA_Flag and Opt_Flag:
            print('Finish Testing the C-Decision scheme with Binary Feedback with Optimal Scheme!')
            ...
            if RA_Flag:
                print('Finish Testing the C-Decision scheme with Binary Feedback '
                ...
                print('Finish Testing only the C-Decision scheme with Binary Feedback')
                return Expect_Return, Reward, Per_V2V_Rate, Per_V2I_Rate, \
                       Per_V2B_Interference

    def robust_run(self, num_episodes, num_test_step, opt_flag, feedback_interval, input_noise_level, fb_noise_level):
        # define robust_run() to test the robustness of the trained C-Decision model
        ...
        # Define the model
        BS_model = Model(inputs=[BS_Input], outputs=[BS_output])
        ...
        for Episode_loop in range(self.num_Episodes):
            ...
            for Run_loop in range(self.num_Test_Step):
                ...
                # we assume that optimal scheme uses the exact states without any errors
                if Opt_Flag:
                    # implement Optimal scheme
                    ...
                    if Input_Noise_Level == 0:
                        ...
                        if FB_Noise_Level > 0:
                            ...
                            for D2D_loop in range(self.num_D2D):
                                ...
                                # add feedback noise for the output of each D2D
                            ...
                            # use Noisy BS Input to get the noisy BS output
                        ...
                        if FB_Noise_Level == 0:
                            ...
                            for D_loop in range(self.num_D2D):
                                ...
                                # find the current channel information from current "States"
                                ...
                                for CH_loop in range(self.num_CH):
                                    # add noise to channel information
                                ...
                                # there is no need to add the noise to D2D's transmit power
                            ...
                            # use the noisy states to generate Q(States, a)
                            Q_Pred = self.brain.predict_one_step(Noisy_States)

                            # use the noisy input states as current states to evaluate RA
                            States = Noisy_States
                            ...
                            # # the correct input shape for each D2D
                            D1_Noisy_State = np.reshape(Noisy_D2D_States[0, :], [1, Num_One_D2D_Input])
                            D2_Noisy_State = np.reshape(Noisy_D2D_States[1, :], [1, Num_One_D2D_Input])
                            D3_Noisy_State = np.reshape(Noisy_D2D_States[2, :], [1, Num_One_D2D_Input])
                            D4_Noisy_State = np.reshape(Noisy_D2D_States[3, :], [1, Num_One_D2D_Input])
        ...
        if RA_Flag and Opt_Flag:
            print('Finish ROBUST Test of the Binary-FB C-Decision scheme with Optimal Scheme!')
            return Expect_Return, Reward, Per_V2V_Rate, Per_V2I_Rate, \
                   Per_V2B_Interference, \
                   RA_Expect_Return, RA_Reward, RA_Per_V2V_Rate, RA_Per_V2I_Rate, \
                   RA_Per_V2B_Interference, \
            ...
            if RA_Flag:
                print('Finish ROBUST Test of the Binary-FB C-Decision scheme withOUT Optimal Scheme!')
                return Expect_Return, Reward, Per_V2V_Rate, Per_V2I_Rate, \
                       Per_V2B_Interference,\
                       RA_Expect_Return, RA_Reward, RA_Per_V2V_Rate, RA_Per_V2I_Rate, \
                       RA_Per_V2B_Interference
            else:
                print('Finish ROBUST Test of the Binary-FB C-Decision scheme!')
                return Expect_Return, Reward, Per_V2V_Rate, Per_V2I_Rate, \
                       Per_V2B_Interference
