...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height):
        ...
        self.fixed_v2v_power_index = 1     # prefixed V2V power index
        ...
        while(i < len(self.vehicles)):
            ...
            if self.vehicles[i].direction == 'u':
                ...
                for j in range(len(self.left_lanes)):
                    ...
                    if (self.vehicles[i].position[1] <= self.left_lanes[j]) \
                            and ((self.vehicles[i].position[1] + delta_distance) >= self.left_lanes[j]):   # came to an cross
    ...
    def compute_reward_with_channel_selection(self, actions_ch_sel):
        ...
        actions = actions_ch_sel   # the channel selection matrix
