# implement the Testing main function for the D-Decision scheme with Binary Feedback
...
def main():
    ...
    num_bs_output_set = [16, 36, 80, 100]
    gamma_set = [0.05]
    ...
    for BS_loop in range(num_BS_Out):
        ...
        for test_loop in range(num_test_settings):
            ...
            train_show_tra = '----- Start the Number -- ' + str(test_num[test_loop]) + ' -- Seed - Testing -----!'
...
def load_trained_model(Env, curr_RL_Config):
    ...
    curr_sim_set = 'Train-Result' + '-SignFB-' + str(Num_D2D_feedback) + '-Batch-' + str(BATCH_SIZE) \
...
def run_test(curr_RL_Config, BS_Agent, test_seed_sequence):
    ...
    if Opt_Flag:
        ...
        curr_sim_set = 'Opt-Run-Result' + '-SignFB-' + str(Num_D2D_feedback) + '-Batch-' + str(Batch_Size) \
                       + '-Gamma-' + str(GAMMA) + '-BSOutput-' + str(Num_BS_Output) \
        ...
        print('To Run Test RL-DNN without Implementing Optimal Scheme!')
        ...
        #  save the tested results to files with their corresponding simulation parameter settings
        curr_sim_set = 'Run-Result' + '-SignFB-' + str(Num_D2D_feedback) + '-Batch-' + str(Batch_Size) \
                       + '-Gamma-' + str(GAMMA) + '-BSOutput-' + str(Num_BS_Output) \
                       + '-Seed-' + str(test_seed_sequence) + '-V2Iweight-' + str(V2I_Weight)
        folder = os.getcwd() + '\\' + curr_sim_set + '\\'
        if not os.path.exists(folder):
